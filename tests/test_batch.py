...
def test_make_batches_pvnet_v1(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            ...
            == pd.Timestamp(datetime.now(tz=timezone.utc)).ceil('5T').replace(tzinfo=None).isoformat()
        )
        ...
        assert (
            ...
            == pd.Timestamp(datetime.now(tz=timezone.utc)).ceil('30T').replace(tzinfo=None).isoformat()
        )
