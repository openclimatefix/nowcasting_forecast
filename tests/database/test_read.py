...
def test_read(db_session):
    ...
    assert forecast_read.gsp_id == forecasts[0].gsp_id
    assert forecast_read.t0_datetime_utc == forecasts[0].target_datetime_utc
    assert forecast_read.target_datetime_utc == forecasts[0].target_datetime_utc
...
def test_read_target_datetime_utc(db_session):
    ...
    forecasts_read = get_forecast(session=db_session, target_datetime_utc=forecasts[0].target_datetime_utc)
